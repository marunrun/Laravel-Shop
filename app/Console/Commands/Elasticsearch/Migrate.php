...
use App\Console\Commands\Elasticsearch\Indices\EsIndexInterface;
...
class Migrate extends Command
{
    ...
    public function handle()
    {
        $this->es = app('es');
        // 索引类数组
        /** @var EsIndexInterface[] $indices */
        $indices = [ProductIndex::class];
        // 遍历索引类数组
        foreach ($indices as $indexClass) {
            ...
            try {
                ...
                $this->warn("更新失败,准备重建".$exception->getMessage());
     ...
     * 创建新的索引
     * @param $aliasName
     * @param  EsIndexInterface  $indexClass
     */
     ...
     * @throws Exception
     */
    private function reCreateIndex($aliasName, $indexClass)
    {
        // 获取索引信息，返回结构化的key为索引名称 value为别名
